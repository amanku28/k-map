    ...
    for t in range(0, 16):
        if (multicombo2[t] == 2):
            ...
            for j in range(0, 4):
                ...
                if ((k[t] and k[t + driver_2[t][j]]) and (
                        (already_combined[t] != 0) and (already_combined[t + driver_2[t][j]] != 0))):
                    duo = []
                    duo.append(t)
                    duo.append(t + driver_2[t][j])
                    duo.sort()
    ...
    for t in range(0, 16):
        ...
        if (multicombo4[t] == 1):
            for j in range(0, 6):
                ...
                if ((k[t] and k[driver_4[t][j][0] + t] and k[driver_4[t][j][1] + t] and k[driver_4[t][j][2] + t] and (
                        already_combined[t] != 0))):
                    quatro = []
                    mini.append(
        ...
        for t in range(0, 16):
            ...
            if (multicombo4[t] == xl):
                ...
                for j in range(0, 6):
                    ...
                    if ((k[t] and k[driver_4[t][j][0] + t] and k[driver_4[t][j][1] + t] and k[
                        driver_4[t][j][2] + t] and (
                                 (already_combined[t] != 0) ^ (already_combined[driver_4[t][j][0] + t] != 0) ^ (
                                 already_combined[driver_4[t][j][1] + t] != 0) ^ (
                                         already_combined[driver_4[t][j][2] + t] != 0)))):
                        quatro = []
                        mini.append(
                            [t, driver_4[t][j][0] + t, driver_4[t][j][1] + t,
                             driver_4[t][j][2] + t])
    ...
    while (max(already_combined) >= 1):
        ...
        for i in range(0, 16):
            if ((combo[i] == 2) and (timer > 2)):
                ...
                for j in range(0, 4):
                    ...
                    if ((k[i] and k[i + driver_2[i][j]]) and (already_combined[i] != 0) and zero[
                        i + driver_2[i][j]] == 0):
                        duo = []
                        duo.append(i)
        ...
        if (k_max == 8):  # minimization for eight squares combination.
            ...
            for j in range(0, 4):
                ...
                if ((map_copy[max_thesis] and map_copy[max_thesis + driver_2[max_thesis][j]]) and (
                (already_combined[max_thesis] != 0))):
                    duo = []
